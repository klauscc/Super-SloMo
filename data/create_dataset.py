...
def extract_frames(videos, inDir, outDir):
    ...
    for video in videos:
        ...
        retn = os.system('{} -i {} -vf scale={}:{} -vsync 0 -qscale:v 2 {}/%04d.jpg'.format(
            os.path.join(args.ffmpeg_dir, "ffmpeg"), os.path.join(inDir, video), args.img_width,
            args.img_height, os.path.join(outDir,
...
def create_clips(root, destination):
    ...
    for file in files:
        ...
        for imageCounter, image in enumerate(images):
            ...
            move("{}/{}/{}".format(root, file, image),
                 "{}/{}/{}".format(destination, folderCounter, image))
...
def main():
    ...
    if (args.dataset == "adobe240fps"):
        ...
        testIndices = random.sample(range(len(videos)),
                                    int((args.train_test_split[1] * len(videos)) / 100))
