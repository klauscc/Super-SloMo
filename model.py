...
def get_time(indices, device):
    """get the time of the intermediate frames.

    Args:
        indices (tensor): Shape (bs,). Indices corresponding to the intermediate 
            frame positions of all samples in the batch.

    Returns: 
        (List). The time and inverse time of the intermediate frame. Each element
            is of shape (bs,). 
    """
    ...
    tt_inverse = torch.tensor(tt_inverse).to(device)
...
def getFlowCoeff(indices, device):
    ...
    C01 = (t[ind]) * (t[ind])
    C10 = (1 - (t[ind])) * (1 - (t[ind]))
    return torch.Tensor(C00)[None, None, None, :].permute(
        3, 0, 1, 2).to(device), torch.Tensor(C01)[None, None, None, :].permute(
            3, 0, 1, 2).to(device), torch.Tensor(C10)[None, None, None, :].permute(
                3, 0, 1, 2).to(device), torch.Tensor(C11)[None, None, None, :].permute(3, 0, 1,
                                                                                       2).to(device)
...
def getWarpCoeff(indices, device):
    ...
    C0 = 1 - t[ind]
    C1 = t[ind]
    return torch.Tensor(C0)[None, None, None, :].permute(
        3, 0, 1, 2).to(device), torch.Tensor(C1)[None, None, None, :].permute(3, 0, 1, 2).to(device)
